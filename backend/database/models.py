...
class PermissionMain(db.Model):
    __tablename__ = 'Permission_Main'
    
    pid = db.Column(db.Integer, primary_key=True)
    msid = db.Column(db.String(10), nullable=False, unique=True)
    mstitle = db.Column(db.String(50), nullable=False)
    bmodid = db.Column(db.String(50), nullable=False)
    bmoddate = db.Column(db.DateTime, default=datetime.now, onupdate=datetime.now)
    
    details = relationship("PermissionDetail", backref="main", cascade="all, delete-orphan")

class PermissionDetail(db.Model):
    __tablename__ = 'Permission_Detail'
    
    detail_id = db.Column(db.Integer, primary_key=True)
    pid = db.Column(db.Integer, ForeignKey('Permission_Main.pid', ondelete='CASCADE'), nullable=False)
